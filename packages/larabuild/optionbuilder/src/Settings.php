...
class Settings
{
    ...
    public  function set(string $section = NULL,  string $key = NULL, array|string $value)
    {
        if (is_array($value)) {
            ...
            if (!$this->isNonSanitizeField($section, $key))
    ...
    public  function resetSection($key = false)
    {
        ...
        if (!empty($key)) {
            ...
            DB::table(config('optionbuilder.db_prefix') . 'settings')->where('section', '<>', 'admin_settings')->delete();
        }
        if (config('cache', true)) {
            Cache::forget('optionbuilder__settings');
        }
    ...
    private function isNonSanitizeField($section, $key): bool
    {
        if (!empty($this->nonSanitizeFields[$section])) {
            if (!empty($key) && in_array($key, $this->nonSanitizeFields[$section])) {
                return true;
            } else {
                return false;
            }
