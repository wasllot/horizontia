...
<div class="op-tooltip">
    <input type="{{ $type == 'editor' ? 'text' : $type }}" @if(!empty($parent_rep)) data-parent_rep="{{$parent_rep}}"
        @endif data-id="{{ $id ?? '' }}" name="{{ $name }}" value="{{ $value ?? '' }}"
    ...
    <div class="form-group-half">
        <div class="op-textcontent">
            <div class="op-tooltip">
                ...
                @endif
                <input type="{{ $type == 'editor' ? 'text' : $type  }}" @if(!empty($parent_rep))
                    data-parent_rep="{{$parent_rep}}" @endif data-id="{{ $id ?? '' }}" name="{{ $name }}"
                    value="{{ $value ?? '' }}"
                    class="op-input-field form-control {{ $class ?? '' }} {{ $type == 'editor' ? 'op-editor' : ''}} {{ !empty($put_env) ? 'put-to-env' : '' }}"
                    placeholder="{{ $placeholder ?? '' }}" 
                    @if(isset($min) && $type == 'number')
                        min="{{ $min ?? 0 }}"
                    @endif
                    @if(isset($max) && $type == 'number')
                        max="{{ $max ?? 0 }}"
                    @endif
                    >
            </div>
