...
<div class="pb-bodywrapper">
    ...
    @component('pagebuilder::components.sidebar',
        ['componentTabs' => $componentTabs, 'components'=>$components,'page'=>$page])
    @endcomponent
    
    <main class="pb-main larabuild-grid1">
        <iframe id="pagebuilder-iframe" allowfullscreen="1" src="{{ URL('pages/'.$page->id.'/iframe') }}" frameborder="0"></iframe>
    </main>
</div>
@endsection
...
@if ($components)
    @foreach ($components as $item)
        @component('pagebuilder::components.grid',
            ['class'=>'d-none sectionable', 'template_id'=>$item['settings']['id'],'id'=>'template_'.$item['settings']['id'],'droppable'=>false])
...
<script>
    ...
    let pb_prefix = "{{config('pagebuilder.url_prefix')}}";
    if( pb_prefix != '' ){
        pb_prefix = pb_prefix+'/';
    }
    let ob_prefix = "{{config('optionbuilder.url_prefix')}}";
    if( ob_prefix != '' ){
        ob_prefix = ob_prefix+'/';
    }
    ...
    function extractJs(section){
        ...
        $(iframe).find('script').each(function(){
            var sectionAttr = $(this).attr('section');
            if (typeof sectionAttr !== typeof undefined && sectionAttr !== false && sectionAttr == section) {
                var text = $(this).text();
                eval(text);
            } 
        });
    ...
    function initBuilderJs() {
        ...
        new Sortable.create(grids, {
            ...
            onStart:function(evt){
                $(evt.item).find('.pb-themesection').hide();
                $('.pb-tooltip.row').addClass('pb-hidetooltip');
            },
            setData: function (dataTransfer, dragEl) {
                // Create the clone (with content)
                dragGhost = dragEl.cloneNode(true).querySelector('.grid-placeholder');
                // Stylize it
                dragGhost.classList.add('pb-ghost');
                // Place it into the DOM tree
                document.body.appendChild(dragGhost);
            ...
            onEnd: function (evt) {
                dragGhost.parentNode.removeChild(dragGhost);
                $(evt.item).find('.pb-themesection').show(); 
    ...
    function setPageSettings(showDemoWarning=false){
        var mainArray = {
            'grids':[],
            'section_data':[],
        };
                ...
                success:function(data){
                    ...
                    if( data.success ){
                        ...
                        if(section_id){
                            ...
                            if(directory){
                                $('#pagebuilder-iframe')[0].contentWindow.dipatchSectionJs(directory);
                            }  
                        }
                        window.unsavedChanges = false;
    ...
    function getSectionSettings(sectionId, getDemoSettings=0) {
        ...
        if (directory) {
            ...
            $.ajax({
                type: 'POST',
                url: `{{ url('${pb_prefix}get-section-settings') }}`,
                data: {
                    'directory': directory,
                    'page_id': pageId,
                    'id': sectionId,
                    'grid_id': gridId,
                    'get_demo_settings': getDemoSettings
                },
                dataType: 'json',
                success: function(data) {
                    if (data.type == 'success') {
                        ...
                        if (data.settings) {
                            ...
                            $('#section-settings-wrapper').html('<span class="at-empty-settings">{{ __("pagebuilder::pagebuilder.no_settings") }}</span>');
                        }
                ...
                error:function(data){
                    showAlert({
                        message     : '{{ __("pagebuilder::pagebuilder.alert_error_message") }}',
                        type        : 'error',
    ...
    window.onload = (event) => {
        jQuery(document).ready(function() {
            ...
            $(document).on('click', '.op-colorpicker .cp-buttons .btn , .cp-cont .cp-buttons .btn',  function(){
                setPageSettings();
            });
            ...
            $(document).on('change', '.op-uploads-img-data input[type=file]', function(event){
                ...
                event.target.value = '';
                $.ajax({
                    url: `{{ url('${ob_prefix}option-builder/upload-files') }}`,
                    method: 'post',
                    contentType: false,
                    processData: false,
                    data:  formData,
            ...
            $(window).on('beforeunload', function () {
                if (window.unsavedChanges) {
                    return '{{ __("pagebuilder::pagebuilder.changes_not_saved") }}';
                }
            });
        });
