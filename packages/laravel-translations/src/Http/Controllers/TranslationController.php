...
class TranslationController extends BaseController
{
    ...
    public function store(Request $request): RedirectResponse
    {
        if (isDemoSite()) {
            return redirect()->route('ltu.translation.index')->with('notification', [
                'type' => 'error',
                'body' => 'You can\'t add/edit anything on the demo site',
            ]);
        }
    ...
    public function import()
    {
        ...
        try {
            ...
            chdir(base_path());
            $command = 'php -d memory_limit=-1 -d max_execution_time=0 artisan translations:import --no-interaction 2>&1';
          	exec($command, $output, $returnVar);

            if($returnVar != 0){
                Log::error('Failed to import translations: ' . implode("\n", $output));
                return redirect()->route('ltu.translation.index')->with('notification', [
                    'type' => 'error',
                    'body' => 'Failed to import translations',
                ]);
            }
            return redirect()->route('ltu.translation.index')->with('notification', [
                'type' => 'success',
            ...
            Log::error('Failed to import translations: ' . $e->getMessage());
            return redirect()->route('ltu.translation.index')->with('notification', [
                'type' => 'error',
                'body' => 'Failed to import translations',
            ]);
