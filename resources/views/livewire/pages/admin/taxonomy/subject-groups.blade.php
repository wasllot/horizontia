<main class="tb-main tb-subjectgroupwrap">
    <div class ="row">
        <div class="col-lg-4 col-md-12 tb-md-40">
            <div class="tb-dbholder tb-packege-setting">
                <div class="tb-dbbox tb-dbboxtitle">
                    @if($editMode)
                        <h5> {{ __('taxonomy.update_subject_group') }}</h5>
                    @else
                        <h5> {{ __('taxonomy.add_subject_group') }}</h5>
                    @endif
                </div>
                <div class="tb-dbbox">
                            ...
                            <div class="tk-themeform__wrap">
                                ...
                                @if($editMode)
                                    <div class="form-group">
                                        ...
                                        <div class="tb-email-status">
                                            <span>{{__('taxonomy.subject_group_status')}}</span>
                                ...
                                <div class="form-group tb-dbtnarea">
                                    <a href="javascript:void(0);" wire:click.prevent="update" class="tb-btn ">
                                        @if($editMode)
                                            {{ __('taxonomy.update_subject_group') }}
                                        @else
                                            {{ __('general.add_now') }}
                                        @endif
                                    </a>
        ...
        <div class="col-lg-8 col-md-12 tb-md-60">
            <div class="tb-dhb-mainheading">
                <h4> {{ __('taxonomy.subject_groups') }}</h4>
                <div class="tb-sortby">
                    <form class="tb-themeform tb-displistform">
                        <fieldset>
                            <div class="tb-themeform__wrap">
                                <div class="tb-actionselect">
                                    <a href="javascript:;" class="tb-btn btnred {{ $selectedSubjectGroups ? '' : 'd-none' }}"  @click="$wire.dispatch('showConfirm', { action : 'delete-subject-group' })" >{{ __('general.delete_selected') }}</a>
                                </div>
                                <div class="tb-actionselect">
                                    <div class="tb-select">
            ...
            <div class="tb-disputetable tb-pageslanguage">
                @if(!empty($subjectGroups) && $subjectGroups->count() > 0)
                    <table class="table tb-table tb-dbholder @if(setting('_general.table_responsive') == 'yes') tb-table-responsive @endif">
                        ...
                        <tbody>
                            @foreach($subjectGroups as $single)
                                <tr>
                                    <td data-label="{{ __('taxonomy.name') }}">
                                        <div class="tb-checkboxwithimg">
                                            <div class="tb-checkbox">
                                                <input id="subject_group_id{{ $single->id }}" wire:model.lazy="selectedSubjectGroups" value="{{ $single->id }}" type="checkbox">
                                                <label for="subject_group_id{{ $single->id }}">
                                                    <span>
                                                        {!! $single->name !!}
                                                    </span>
                                                </label>
                                    ...
                                    <td data-label="{{__('general.actions')}}">
                                        <ul class="tb-action-icon">
                                            ...
                                            <li> <a href="javascript:void(0);" @click="$wire.dispatch('showConfirm', { id : {{ $single->id }}, action : 'delete-subject-group' })"  class="tb-delete"><i class="icon-trash-2"></i></a> </li>
                                        </ul>
                                    </td>
                                </tr>
                            @endforeach
                        ...
                        {{ $subjectGroups->links('pagination.custom') }}
                    @else
